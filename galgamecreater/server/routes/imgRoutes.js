...
router.post('/', upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        const newKey = 'bg' + (lastCount); // 新键的命名方式可以根据需求进行修改
        const newValue = 'images/' + req.file.filename;

        // 在现有backgroundimg内容的基础上添加新的键值对
        currentBackgroundImg[newKey] = newValue;

        // 更新jsonDataCopy的backgroundimg对象为包含新旧内容的对象
        jsonDataCopy.backgroundimg = currentBackgroundImg;
